...
npm run dev:slow                    # 1往復300ms。DB_LATENCY_MS=500 npm run dev:slow のように変えられる
npm run e2e:lag                     # ↑で起動した状態で、動かした積み木がワープしないかを確かめる
```
...
- 相手のカーソルが、名前と色つきで紙の上に見える（相手が盤の外に出ても、最後の位置に残ります）。**大きさは画面基準**で、拡大縮小しても変わりません（道具箱も同じ）
- 左上に「いま一緒に開いている人」が出る（開いた瞬間から。色つきの点 = カーソルを動かした人）
...
協力してくれるユーザーさん・VTuberさんの名簿です（`/contacts`、盤の左上「連絡先」から）。
積み木（仕事）とは別の軸で、**期間をまたいで残ります**。

| 持つもの | 中身 |
|---|---|
| 種類 | ユーザー / VTuber / その他 |
| 段階 | 候補 → 声かけ済み → 返事待ち → 協力中（または 見送り） |
| 連絡手段 | X・Discord・メール・ページ（あるものだけ表示） |
| 担当・最後に連絡した日・メモ | |

- 上のタブ（**すべて / VTuber / ユーザー / その他**）で切り替え。タブを開いた状態で足すと、その種類になります
- 全部、行の上で触れます: **名前を押すと書きかえ**、**連絡先は名前のすぐ横**（押すと書きかえ・× で消す・＋ で足す）、**担当のマークを押すと選択**、種類の札・段階の札も同様
- ＋ を押すと**すぐ入力欄**。貼った文字から **X / Discord / メール / ページ を自動で見分けて**左の札に出します（X のURLならIDだけ、`youtube.com/...` なら https を付けて保存）
- **札を押すと候補の小窓**（X / Discord / メール / ページ）。自動判定が違っていても選び直せます。既にある連絡先も、ラベル（「X ▾」など）を押せば同じ小窓で種類を変えられます。既に入っている種類には「上書き」の印
- 閉じているときは連絡先を**2つまで**見せ、あふれた分は「+N」にまとめます。行の空いている所（か ▾）を押すと開いて全部見え、**備考**が出ます
- 段階を「声かけ済み」「返事待ち」にすると、**その日が「最後に連絡した日」に自動で入ります**
- 並びは**返事待ちが一番上**。相手のボールで忘れやすいものを上に置いています
