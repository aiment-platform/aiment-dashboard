...
# 追補 — 本番で約7秒後に一瞬戻る (2026-09-24)

## 症状
本番(Neon)で積み木を動かすと、動かした位置が約7秒保たれたあと**一瞬元に戻り**、また動かした位置へ戻る。

## 原因
一時メモリの記録を「書けてから5秒で捨てる」**時間頼み**にしていた。遠い DB では書き込み(約2秒)+5秒の間に
画面用のデータが届かないことがあり、捨てた瞬間だけ DB の古い位置が描かれた(2+5≒7秒)。

## 直し: 時間ではなく DB の記録で捨てる
- `moveBlocks` が `updated_at` を書き、その時刻を返す(`stackBlocksAction` → `{ written_at }`)
- `PeriodBlock` に `updated_at` を足した
- 記録は `writtenAt` を持ち、捨てるのは次のときだけ:
  1. 届いたデータが動かした位置と一致した
  2. 届いたデータの `updated_at` が自分の `writtenAt` より新しい(=あとから誰かが書いた)
- 上に載っている積み木は座標を持たない(親から計算)ので、座標は比べない(`m.x === null` なら無視)
- **タイマーは撤去**。データが届かない限り記録は残る(残っていても、それは自分が書いた正しい位置)

## 検証
手元では書き込みの返事に最新データが乗るので再現しない。**`stackBlocksAction` の `refresh()` を一時的に外して**
「データが届くのが遅れる」本番の状況を作り、12秒見張った:

| | 元の位置に戻った回数 |
|---|---|
| 直す前(5秒で捨てる) | 126回(5.1秒〜) |
| 直した後 | 0回 |

相方があとから同じ積み木を動かした場合は、相方の位置が勝つことも確認(ルール2)。


---

# 追補 — ページがどれも遅い (2026-09-24)

## 原因
Neon は **シンガポール(sin1)**。Vercel の関数は何も指定しないと **アメリカ東海岸(iad1)** で動く。
関数 ↔ DB が太平洋を挟むと1往復 ≒ 230ms。1ページで何往復も順番に聞くので、それが積み上がる。

## 直し
1. **`vercel.json` で関数を sin1 に固定**(`"regions": ["sin1"]`)。DB と同じ場所。これが本命
2. **往復の回数を減らす**
   - 盤(`getPeriodBoard`): 期間 → ワークストリーム → 積み木 → 残り、の4往復を **JOIN で1往復**に
   - 一覧(`listWork`): 7つを順番に → `Promise.all` で同時に
3. **`loading.tsx`**(盤と `(app)`)— 押した瞬間に画面が切り替わり、中身があとから埋まる

## 実測(`DB_LATENCY_MS` で往復を再現)

| | 直す前・230ms | 直した後・230ms | 直した後・2ms(sin1同士) |
|---|---|---|---|
| 盤 `/` | 1.21秒 | 0.53秒 | **0.05秒** |
| 一覧 `/tasks` | 1.67秒 | 0.27秒 | **0.05秒** |
| 連絡先 | 0.29秒 | 0.29秒 | **0.05秒** |

**関数がどこで動いているかは、レスポンスヘッダー `x-vercel-id` で分かる**
(`hnd1::sin1::…` なら、入口が東京・関数がシンガポール)。
